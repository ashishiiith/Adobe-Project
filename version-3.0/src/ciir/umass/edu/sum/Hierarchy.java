...
public class Hierarchy {
	...
	private NPExtractor npe = null;
        private Map<String, Integer> tm = new TreeMap<String, Integer>(); //store aspect phrase count
	...
	public Hierarchy(IndriSearchEngine se, boolean hashtag)
    {
        this.se = se;
	...
	public void estimate(String query, int topD) throws Exception
	{
		...
		{
			System.out.println("Query term: " + qterms[i]);
			qts.add(qterms[i]);
		}
	...
	{
		int[] internalIDs = se.docInternalIDs(docExternalIDs);
		estimate(qTerms, internalIDs, docScores);
	}
	public void estimate(List<String> qTerms, int[] docInternalIDs, double[] docScores) throws Exception
	{
		DocumentVector[] dvs = se.getDocumentVectors(docInternalIDs);
		estimate(qTerms, dvs, docScores);
	}
	private void estimate(List<String> qTerms, DocumentVector[] dvs, double[] scores) throws Exception
	{
		...
		sorted_map = MapSort.sortByValue(tm);
		//System.out.println(MapSort.sortByValue(tm));
	 ...
	 * @return
	 */
	private LanguageModel buildDocumentLM(DocumentVector dv, List<String> queryTerms, HashMap<String, Integer> minDistanceToQT, DocumentPhraseVector dpv)
	{
		...
		if(usePhrases)
		{
			...
			for(int i=0;i<nps.size();i++)
			{
				...
				//else
					...
					//else
					//	System.out.println(nps.get(i).text);
                		}
            		}
			...
			for(int i=0;i<nps.size();i++)
			{
				...
				if((!usePhrasesOnly || np.text.indexOf(" ")!=-1) && !hashTags)
				{
                    			if (tm.containsKey(nps.get(i).text))
                        			...
                        			tm.put(nps.get(i).text, 1);
					//System.out.println("Candidate phrases: " + np.text);
	 ...
	 * @return
	 */
	private Neighbor[][] extractTermCoocurrenceStats(Hashtable<String, Integer> vocabIndex, DocumentVector[] dvs, DocumentPhraseVector[] dpvs)
	{
	...
	//Writing extra filter function
    private boolean filter(String key) {
        //check if stop word exist in each tag
